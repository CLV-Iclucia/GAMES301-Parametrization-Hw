...
void MainViewerWidget::TutteParamCirUniform() const
{
	meshviewerwidget->PerformTutte(true, false);
	std::cout << "Perform Tutte parametrization" << std::endl;
}

void MainViewerWidget::TutteParamCirFloater() const
{
	meshviewerwidget->PerformTutte(true, true);
	std::cout << "Perform Tutte parametrization" << std::endl;
}

void MainViewerWidget::TutteParamSqrUniform() const
{
	meshviewerwidget->PerformTutte(false, false);
	std::cout << "Perform Tutte parametrization" << std::endl;
}

void MainViewerWidget::TutteParamSqrFloater() const
{
	meshviewerwidget->PerformTutte(false, true);
	std::cout << "Perform Tutte parametrization" << std::endl;
}
...
void MainViewerWidget::CreateParamWidget(void)
{
	...
	connect(meshparamwidget, SIGNAL(PerformTutteSignalCirUniform()), SLOT(TutteParamCirUniform()));
	connect(meshparamwidget, SIGNAL(PerformTutteSignalCirFloater()), SLOT(TutteParamCirFloater()));
	connect(meshparamwidget, SIGNAL(PerformTutteSignalSqrUniform()), SLOT(TutteParamSqrUniform()));
	connect(meshparamwidget, SIGNAL(PerformTutteSignalSqrFloater()), SLOT(TutteParamSqrFloater()));
}
